...
import io.ezalabs.multiversxspringbootstarterreactive.config.constants.WalletConstants;
import io.ezalabs.multiversxspringbootstarterreactive.error.exception.CannotDeriveKeysException;
import io.ezalabs.multiversxspringbootstarterreactive.error.exception.CannotGenerateMnemonicException;
...
public class MnemonicsUtils {
  ...
  private static byte[] mnemonicToBip39Seed(String mnemonic) {
    ...
    var passphrase = WalletConstants.BIP39_SALT_MODIFIER.getBytes();
    var generator = new PKCS5S2ParametersGenerator(new SHA512Digest());

    generator.init(mnemonicBytes, passphrase, WalletConstants.BIP39_PBKDF2_ROUNDS);
    return ((KeyParameter) generator.generateDerivedParameters(512)).getKey();
  }

  private static byte[] generateEntropy() {
    var random = new SecureRandom();
    var entropy = new byte[WalletConstants.DEFAULT_ENTROPY_BITS / 8];
  ...
  private static byte[] bip39SeedToPrivateKey(byte[] seed, long accountIndex) throws IOException {
    ...
    var derivationPath = Arrays.copyOf(WalletConstants.MX_DERIVATION_PATH, WalletConstants.MX_DERIVATION_PATH.length);

    derivationPath[derivationPath.length - 1] = accountIndex;

    for (var segment : derivationPath) {
      keyAndChainCode = ckdPriv(key, chainCode, segment + WalletConstants.HARDENED_OFFSET);
  ...
  private static KeyAndChainCode bip39SeedToMasterKey(byte[] seed) {
    var result = hmacSHA512(WalletConstants.BIP32_SEED_MODIFIER.getBytes(), seed);
